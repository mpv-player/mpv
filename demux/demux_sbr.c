...
static const struct format_codec_info FMT_TO_CODEC[] = {
    ...
    [SBR_SUBTITLE_FORMAT_WEBVTT ] = {"subrandr/vtt",  "WebVTT"},
};

struct sub_ext_codec {
    const char *ext;
    struct format_codec_info codec_info;
};

static const struct sub_ext_codec EXT_CODECS[] = {
...
static int demux_open_sbr(struct demuxer *demuxer, enum demux_check check)
{
    ...
    for (const struct sub_ext_codec *ext = EXT_CODECS; ext->ext; ++ext) {
