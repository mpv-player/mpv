...
static void thread_feed(struct ao *ao)
{
    ...
    int frames_written = ao_read_data(ao, (void **)data, frame_count,
                                      mp_time_us() + (int64_t)llrint(delay_us));

    // note, we can't use frames_written here since we already commited to
    // frame_count above in the GetBuffer call
    hr = IAudioRenderClient_ReleaseBuffer(state->pRenderClient,
                                          frame_count, 0);
    EXIT_ON_ERROR(hr);
    if (frames_written != frame_count) {
        MP_VERBOSE(state, "only %d of %d frames written\n",
