...
static void process_dst(struct mp_filter *da, struct demux_packet *mpkt)
{
    ...
    if (p->avframe->format != AV_SAMPLE_FMT_DSD ||
...
static bool init_dst_decoder(struct mp_filter *da)
{
    ...
    p->avctx->request_sample_fmt = AV_SAMPLE_FMT_DSD;
    mp_chmap_to_av_layout(&p->avctx->ch_layout, &p->chmap);

    if (avcodec_open2(p->avctx, dec, NULL) < 0 ||
        p->avctx->sample_fmt != AV_SAMPLE_FMT_DSD)
        return false;

    return true;
#else
    MP_ERR(da, "FFmpeg is too old to support raw DSD output for DST.\n");
    return false;
#endif
}
