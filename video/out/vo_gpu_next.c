...
#ifdef _WIN32
        // Wayland treats sRGB as having a gamma 2.2 EOTF, while Windows uses the
        // sRGB piecewise function. Send piecewise sRGB to Windows in HDR mode so
        // that it can be converted to PQ, the same way as mpv does internally.
        // Note that in SDR mode, even with ACM enabled, Windows assumes the display
        // is sRGB. It doesn't perform gamma conversion, or any conversions would
        // roundtrip back to sRGB. In which case the EOTF depends on the display.
        // Ideally, compositors would agree on how to handle sRGB, but I’ll leave
        // that part of the story for the reader to explore.
