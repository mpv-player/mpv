...
static void set_colorspace_hint(struct priv *p, struct mp_image *src, struct pl_color_space *hint)
{
    struct ra_swapchain *sw = p->ra_ctx->swapchain;
    struct mp_image_params hint_params = {0};
    if (src)
        hint_params = src->params;


    // Override the color and repr.
    int alpha = 0;
#if PL_API_VER >= 344
    alpha = PL_ALPHA_NONE;
#endif
    hint_params.color = hint ? *hint : pl_color_space_srgb;
    hint_params.repr.sys = PL_COLOR_SYSTEM_RGB;;
    hint_params.repr.levels = p->output_levels;
    hint_params.repr.alpha = p->ra_ctx->opts.want_alpha ? PL_ALPHA_INDEPENDENT : alpha;

    if (sw->fns->set_color) {
        sw->fns->set_color(sw, &hint_params);
        // Wayland on Vulkan requires this until we can do VK_COLOR_SPACE_PASS_THROUGH_EXT
        if (strcmp(p->ra_ctx->fns->name, "waylandvk") == 0)
            pl_swapchain_colorspace_hint(p->sw, hint);
        *hint = hint_params.color;
    } else {
        pl_swapchain_colorspace_hint(p->sw, hint);
    }
}
...
static bool draw_frame(struct vo *vo, struct vo_frame *frame)
{
    ...
    if (target_hint && frame->current) {
        ...
        if (p->icc_profile)
            hint = p->icc_profile->csp;
        set_colorspace_hint(p, frame->current, &hint);
    } else if (!target_hint) {
        if (!hint.hdr.min_luma)
            hint.hdr.min_luma = target_csp.hdr.min_luma;
        set_colorspace_hint(p, frame->current, NULL);
    }
    ...
    if (target_hint && sw->fns->set_color && strcmp(p->ra_ctx->fns->name, "waylandvk") != 0)
        target.color = hint;
