                  ...
                  struct vo_frame *frame)
{
    ...
    // Vulkan needs to transfer the wrapped image back to the client before
    // destroying the temporary pl_tex wrapper.  In particular, done_frame()
    // may enqueue a layout transition and signal the client-provided
    // completion semaphore for presentation.
    if (p->context->fns->done_frame)
        p->context->fns->done_frame(p->context);

    // Destroy the temporary wrapper only after the backend has released it.
