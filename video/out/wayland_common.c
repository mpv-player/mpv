                          ...
                          uint32_t format)
{
    ...
    if (wl->drm_format_ct == wl->drm_format_ct_max) {
        ...
        wl->drm_formats = talloc_realloc(wl, wl->drm_formats, uint32_t, wl->drm_format_ct_max);
    }
...
bool vo_wayland_supported_format(struct vo *vo, uint32_t drm_format) {
    ...
    for (unsigned int i = 0; i < wl->drm_format_ct; ++i) {
                                ...
                                const char *interface, uint32_t ver)
{
    ...
    if (!strcmp (interface, zwp_linux_dmabuf_v1_interface.name) && (ver >= 2) && found++) {
        ...
        wl->drm_formats = talloc_array(wl, uint32_t, wl->drm_format_ct_max);
    }
