...
void mp_dxgi_factory_uninit(struct mp_dxgi_factory_ctx *ctx)
{
    ...
    ctx->color_management_status_monitor = NULL;
                                   ...
                                   HWND hwnd, DXGI_OUTPUT_DESC1 *desc)
{
    ...
    if (!ctx->factory || !IDXGIFactory1_IsCurrent(ctx->factory)) {
        // This also clears `white_level_monitor` and `color_management_status_monitor`,
        // to invalidate cached value.
                                      ...
                                      struct mp_w32_acm_status *status)
{
    ...
    if (ctx && ctx->color_management_status_monitor == desc.Monitor) {
    ...
    if (ctx) {
        ctx->color_management_status_monitor = desc.Monitor;
