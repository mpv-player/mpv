...
#define OPT_BASE_STRUCT struct madvr_opts
const struct m_sub_options madvr_conf = {
    ...
    .defaults = &(struct madvr_opts) {
        .border_background = BACKGROUND_COLOR,
        .inter_preserve = true,
    },
    .size = sizeof(struct madvr_opts),
...
static int preinit(struct vo *vo)
{
    ...
    p->next_opts_cache = m_config_cache_alloc(p, vo->global, &madvr_conf);
    ...
    mppl_log_set_probing(p->pllog, true);
    p->gpu = p->context->gpu;
...
const struct vo_driver video_out_madvr = {
    .description = "Video output based on madshi's Video Renderer",
    .name = "madvr",
