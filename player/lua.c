...
// If a lua c-function does a new allocation (which is not tied to an existing
// context), and a lua-error occures before "free" - the allocation is leaked.
...
//   lua_pushc{function,closure}. This takes care of wrapping it with the
...
// - The autofree runtime "price" (to avoid leaks) is fixed, and typically less
//   than 15% compared to normal new/free (slower function -> smaller price %).
//   Max price is about x2 slower for a no-op function which "needs" new/free.
