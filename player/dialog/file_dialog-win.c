...
static wchar_t *create_extentions_pattern(void *talloc_ctx, char **extension)
{
    if (!extension)
        return NULL;
...
char **mp_file_dialog_get_files(void *talloc_ctx, const mp_file_dialog_params *params)
{
    ...
    hr = CoCreateInstance(save ? &CLSID_FileSaveDialog : &CLSID_FileOpenDialog,
                          NULL, CLSCTX_ALL, &IID_IFileDialog, (void **)&pFileDialog);
    if (FAILED(hr))
        goto done;
    ...
    if (filter && filter->name) {
        ...
        while (filter->name) {
            wchar_t *extensions = create_extentions_pattern(tmp, filter->extensions);
        ...
        if (count)
            IFileDialog_SetFileTypes(pFileDialog, count, file_types);
    }
    ...
    if (params->initial_dir) {
        ...
        if (SUCCEEDED(hr)) {
            IFileDialog_SetFolder(pFileDialog, psi);
            IShellItem_Release(psi);
        }
    }

    if (params->title)
        IFileDialog_SetTitle(pFileDialog, mp_from_utf8(tmp, params->title));

    if (params->initial_selection)
        IFileDialog_SetFileName(pFileDialog, mp_from_utf8(tmp, params->initial_selection));
    ...
    if (params->flags & MP_FILE_DIALOG_MULTIPLE)
        options |= FOS_ALLOWMULTISELECT; // TODO: Add commands and define separator for output
    IFileDialog_SetOptions(pFileDialog, options | FOS_FORCEFILESYSTEM);

    HWND parent = params->parent ? (HWND)(intptr_t)(*(int64_t*)params->parent) : NULL;
    hr = IFileDialog_Show(pFileDialog, parent);
    if (FAILED(hr))
        goto done;
    ...
    if (params->flags & MP_FILE_DIALOG_MULTIPLE) {
        hr = IFileDialog_QueryInterface(pFileDialog, &IID_IFileOpenDialog,
                                        (void **)&pFileOpenDialog);
        if (FAILED(hr))
            goto done;

        hr = IFileOpenDialog_GetResults(pFileOpenDialog, &pItemArray);
        if (FAILED(hr))
            goto done;

        hr = IShellItemArray_GetCount(pItemArray, &itemCount);
        if (FAILED(hr))
            goto done;
    } else {
        itemCount = 1;
    ...
    for (DWORD i = 0; i < itemCount; i++) {
        ...
        if (params->flags & MP_FILE_DIALOG_MULTIPLE) {
            hr = IShellItemArray_GetItemAt(pItemArray, i, &pItem);
            if (FAILED(hr))
                continue;
        } else {
            hr = IFileDialog_GetResult(pFileDialog, &pItem);
            if (FAILED(hr))
                continue;
        }
