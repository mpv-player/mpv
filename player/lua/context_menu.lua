...
local function add_menu(menu_items, x, y)
    ...
    local width = calculate_width(visible_items, osd_w, osd_h, checkbox) +
                  options.padding_x * 2
    -- Limit the height to that of the OSD to prevent freezing with huge menus,
    -- but add padding_y to not draw the bottom rounded corners which would
    -- make it look like the menu ends there.
    local height = math.min(calculate_height(visible_items), osd_h + options.padding_y)
...
local function add_submenu(ass, menu, level, style, background_style)
    ...
    local clipping_coordinates = "0,0," .. menu.x + menu.width .. ",99999"

    items[level] = {}
    local i = 1
    for _, item in ipairs(menu.items) do
        local item_y = menu.y + options.padding_y + (i - 0.5) * line_height

        -- Prevent freezing with many items.
        if item_y > menu.y + menu.height then
            return
        end
        ...
        if item.type == "separator" then
            ...
            local focused = (i == focused_index and level == focused_level) or
                            i == menu.index_with_open_submenu
            ...
            ass:append(style .. "{\\clip(" .. clipping_coordinates .. ")}")
...
mp.register_script_message("open", function ()
    ...
    add_menu(mp.get_property_native("menu-data"), get_scaled_mouse_pos())
