...
local function format_playlist_entry(entry, show, trailing_slash_pattern)
...
mp.add_key_binding(nil, "select-playlist", function ()
    ...
    for i, entry in ipairs(mp.get_property_native("playlist")) do
        playlist[i] = format_playlist_entry(entry, show, trailing_slash_pattern)
...
local function get_active_bindings()
    ...
    for _, binding in pairs(mp.get_property_native("input-bindings")) do
        ...
        ) and binding.section ~= "input_forced_console"
          and binding.section ~= "input_forced_stats" then
...
local function playlist()
    ...
    for i, entry in ipairs(get("playlist")) do
        items[i] = {
            title = format_playlist_entry(entry, show, trailing_slash_pattern),
...
local function update_builtin_submenu(item)
    ...
    local min = (item.builtin_submenu == "$playlist" or
                 item.builtin_submenu == "$editions") and 2 or 1
...
local function parse_menu_item(line)
    ...
    if builtin_submenus[tokens[2]] then
        ...
        if item.builtin_submenu == "$playlist" and
           mp.get_property_number("playlist-count") > 999 then
...
mp.add_key_binding(nil, "context-menu", function (info)
    ...
    if info.event == "up" then
        if use_context_menu_script then
            mp.commandv("script-message-to", "context_menu", "select")
        end
