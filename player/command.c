...
{
    char *path = mp_get_user_path(NULL, mpctx->log, url);
    struct track *t = NULL;
    for (int n = 0; n < mpctx->num_tracks; n++) {
        struct track *track = mpctx->tracks[n];
...
static void cmd_expand_path(void *p)
{
    ...
    cmd->result = (mpv_node){
        .format = MPV_FORMAT_STRING,
        .u.string = mp_get_user_path(NULL, mpctx->log, cmd->args[0].v.s)
    };
}
...
static void cmd_normalize_path(void *p)
{
    ...
    char *path = mp_get_user_path(NULL, mpctx->log, cmd->args[0].v.s);
    cmd->result = (mpv_node){
        .format = MPV_FORMAT_STRING,
...
static void cmd_loadfile(void *p)
{
    ...
    char *path = mp_get_user_path(NULL, mpctx->log, filename);
...
static void cmd_load_config_file(void *p)
{
    ...
    char *config_file = mp_get_user_path(ctx, mpctx->log, cmd->args[0].v.s);
...
static void cmd_load_input_conf(void *p)
{
    ...
    char *config_file = mp_get_user_path(NULL, mpctx->log, cmd->args[0].v.s);
