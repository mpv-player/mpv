                                   ...
                                   int action, void *arg)
{
    ...
    return mp_property_do(name, ka->action, ka->arg, mpctx->command_ctx);
}
                             ...
                             int action, void *arg)
{
    ...
    return mp_property_do(real_property, action, arg, mpctx->command_ctx);
}
                                        ...
                                        int action, void *arg)
{
    ...
    return mp_property_do(real_property, action, arg, cmd);
}
                   ...
                   struct command_ctx *ctx)
{
    ...
    int r = m_property_do(mpctx->log, ctx->properties, name, action, val, mpctx);

    if (mp_msg_test(mpctx->log, MSGL_V) && is_property_set(action, val)) {
        ...
        MP_VERBOSE(mpctx, "Set property: %s%s%s -> %d\n",
                   name, t ? "=" : "", t ? t : "", r);
        talloc_free(t);
    }
    return r;
...
static void show_property_osd(MPContext *mpctx, const char *name, int osd_mode)
{
    ...
    mp_property_do(name, M_PROPERTY_GET_CONSTRICTED_TYPE, &prop, mpctx->command_ctx);
    if ((osd_mode & MP_ON_OSD_BAR)) {
        if (prop.type == CONF_TYPE_INT && prop.min < prop.max) {
            ...
            if (disp.osd_progbar)
                n = disp.marker;
            int i;
            if (mp_property_do(name, M_PROPERTY_GET, &i, mpctx->command_ctx) > 0)
            ...
            if (disp.osd_progbar)
                n = disp.marker;
            float f;
            if (mp_property_do(name, M_PROPERTY_GET, &f, mpctx->command_ctx) > 0)
...
static bool check_property_autorepeat(char *property,  struct command_ctx *ctx)
{
    struct m_option prop = {0};
    if (mp_property_do(property, M_PROPERTY_GET_TYPE, &prop, ctx) <= 0)
        return true;
...
static bool check_property_scalable(char *property, struct command_ctx *ctx)
{
    struct m_option prop = {0};
    if (mp_property_do(property, M_PROPERTY_GET_TYPE, &prop, ctx) <= 0)
        return true;
...
{
    int r = mp_property_do(name, action, arg, cmd->mpctx->command_ctx);
...
static void cmd_cycle_values(void *p)
{
    ...
    struct m_option prop = {0};
    int r = mp_property_do(name, M_PROPERTY_GET_TYPE, &prop, mpctx->command_ctx);
    if (r <= 0) {
        show_property_status(cmd, name, r);
        return;
    }
    ...
    r = mp_property_do(name, M_PROPERTY_GET, &curval, mpctx->command_ctx);
    if (r <= 0) {
        show_property_status(cmd, name, r);
        return;
    }
...
static void cmd_del(void *p)
{
    ...
    int r = mp_property_do(name, M_PROPERTY_DELETE, NULL, mpctx->command_ctx);
...
static void cmd_change_list(void *p)
{
    ...
    struct m_option prop = {0};
    if (mp_property_do(name, M_PROPERTY_GET_TYPE, &prop, mpctx->command_ctx) <= 0) {
    ...
    union m_option_value val = m_option_value_default;
    if (mp_property_do(name, M_PROPERTY_GET, &val, mpctx->command_ctx) <= 0) {
    ...
    if (r >= 0 && mp_property_do(name, M_PROPERTY_SET, &val, mpctx->command_ctx) <= 0)
        r = -1;
    m_option_free(&prop, &val);
    if (r < 0) {
...
static void cmd_add_cycle(void *p)
{
    ...
    if (cmd->cmd->repeated && !check_property_autorepeat(property, mpctx->command_ctx) &&
    ...
    if (check_property_scalable(property, mpctx->command_ctx)) {
        scale = cmd->cmd->scale;
        scale_units = 1;
    }
...
static void cmd_ab_loop(void *p)
{
    ...
    if (mpctx->opts->ab_loop[0] == MP_NOPTS_VALUE) {
        mp_property_do("ab-loop-a", M_PROPERTY_SET, &now, mpctx->command_ctx);
        show_property_osd(mpctx, "ab-loop-a", cmd->on_osd);
    } else if (mpctx->opts->ab_loop[1] == MP_NOPTS_VALUE) {
        mp_property_do("ab-loop-b", M_PROPERTY_SET, &now, mpctx->command_ctx);
...
static void cmd_align_cache_ab(void *p)
{
    ...
    mp_property_do("ab-loop-a", M_PROPERTY_SET, &a, mpctx->command_ctx);
    mp_property_do("ab-loop-b", M_PROPERTY_SET, &b, mpctx->command_ctx);
