...
static mpv_handle *jclient(js_State *J) { return jctx(J)->client; }
...
static void destruct_af_file(void *p) { fclose(*(FILE **)p); }
...
static void destruct_af_dir(void *p) { closedir(*(DIR **)p); }
...
static void destruct_af_mpv_alloc(void *p) { mpv_free(*(char **)p); }
...
static void af_push_file(js_State *J, const char *fname, int limit, void *af) {
  ...
  int flags = STREAM_READ_FILE_FLAGS_DEFAULT | STREAM_ALLOW_PARTIAL_READ |
              STREAM_SILENT;
  bstr data =
      stream_read_file2(filename, af, flags, jctx(J)->mpctx->global, limit);
  if (data.start) {
...
static void script_set_property(js_State *J) {
  int e =
      mpv_set_property_string(jclient(J), js_tostring(J, 1), js_tostring(J, 2));
  push_status(J, e);
}
...
static void script_input_set_section_mouse_area(js_State *J) {
  ...
  mp_input_set_section_mouse_area(
      jctx(J)->mpctx->input, section, jsL_checkint(J, 2),
      jsL_checkint(J, 3),                      // x0, y0
...
static void script_file_info(js_State *J) {
  ...
  const char *stat_names[] = {"mode", "size", "atime", "mtime", "ctime", NULL};
  const double stat_values[] = {statbuf.st_mode, statbuf.st_size,
...
static void pushnode(js_State *J, mpv_node *node) {
  ...
  case MPV_FORMAT_STRING:
    js_pushstring(J, node->u.string);
    break;
  case MPV_FORMAT_INT64:
    js_pushnumber(J, node->u.int64);
    break;
  case MPV_FORMAT_DOUBLE:
    js_pushnumber(J, node->u.double_);
    break;
  case MPV_FORMAT_FLAG:
    js_pushboolean(J, node->u.flag);
    break;
...
static int get_obj_properties(void *ta_ctx, char ***keys, js_State *J,
                              int idx) {
  int length = 0;
...
static const struct fn_entry utils_fns[] = {
    AF_ENTRY(readdir, 2),    FN_ENTRY(file_info, 1),    FN_ENTRY(split_path, 1),
    AF_ENTRY(join_path, 2),  FN_ENTRY(get_env_list, 0),

    FN_ENTRY(read_file, 2),  AF_ENTRY(_write_file, 3),  FN_ENTRY(getenv, 1),
    FN_ENTRY(compile_js, 2), FN_ENTRY(_gc, 1),          {0}};
