    ...
    for (const char *ptr = path; ptr < end; ptr = next + 1) {
        ...
        switch (next - ptr) {
            ...
            case 1:
                ...
                break;
            case 2:
                // Normalizing symlink/.. results in a wrong path. e.g. if the
                // current working directory is /tmp/foo, and it is a symlink
                // to /usr/bin, mpv ../file.mkv opens /usr/file.mkv, so we
                // can't normalize the path to /tmp/file.mkv. Resolve symlinks
                // to fix this. Otherwise we don't use realpath so users can
                // use symlinks e.g. to hide how media files are distributed
                // over real storage and move them while still resuming
                // playback as long as the symlinked path doesn't change.
                if (ptr[0] == '.' && ptr[1] == '.') {
