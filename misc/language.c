...
static const struct lang_iso_639 {
    struct { const char s[3]; uint8_t l; } lang;
} all_langs[] = {
    {L(aar)},
    {L(abk)},
    {L(ace)},
...
static int lang_find(const void *key, const void *lang)
{
    const struct lang_iso_639 *l = lang;
    return bstrcasecmp(*(const bstr*)key, (bstr){(unsigned char *)l->lang.s, l->lang.l});
}

static bool lang_exists(bstr lang)
{
    const struct lang_iso_639 *l = bsearch(&lang, all_langs, MP_ARRAY_SIZE(all_langs),
                                           sizeof(all_langs[0]), &lang_find);
    return l ? true : false;
}
...
bstr mp_guess_lang_from_filename(bstr name, int *lang_start, bool *hearing_impaired)
{
    ...
    // Make sure the primary subtag has a match.
    bool match = true;
    bstr lang = (bstr){name.start + i + 1, lang_length};
    bstr rest = lang;
    bstr primary = bstr_split(rest, "-", &rest);
    bstr canonical_primary = canonicalize(primary);

    if (!bstrcmp(primary, canonical_primary))
        match = lang_exists(canonical_primary);

    if (lang_start && match)
        *lang_start = i;

    return match ? lang : (bstr){0};
}
