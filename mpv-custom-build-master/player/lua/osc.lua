...
local user_opts = {
    ...
    tooltipborder = 2,          -- border of tooltip in bottom/topbar
...
local osc_styles = {
    ...
    box = "{\\rDefault\\blur0\\bord0\\1c&H000000\\3c&HFFFFFF}",

    smallButtonsRotated = "{\\fr-90\\blur.4\\bord0\\1c&HFFFFFF\\3c&HFFFFFF\\fs18\\fnmpv-osd-symbols}",
    smallButtonsBar = "{\\blur.4\\bord0\\1c&HFFFFFF\\3c&HFFFFFF\\fs20\\fnmpv-osd-symbols}",
    timecodesBar = "{\\b600\\blur.4\\bord0\\1c&HFFFFFF\\3c&HFFFFFF\\fs18}",
    timePosBar = "{\\b600\\blur.4\\bord".. user_opts.tooltipborder .."\\1c&HFFFFFF\\3c&H000000\\fs18}",
    vidtitleBar = "{\\b600\\blur.4\\bord0\\1c&HFFFFFF\\3c&HFFFFFF\\fs18\\q2}",
}
...
layouts["topbar"] = function()
    ...
    local tcW = 100
    local tsW = 80
    local minW = (buttonW + padX)*8 + (tcW + padX)*2 + (tsW + padX)*2 + 140
    ...
    local lineH = 18
    local line1 = padY - 1
    local line2 = osc_geo.h - lineH - padY
    ...
    sh_area_y0 = osc_geo.y
    sh_area_y1 = osc_geo.y + osc_geo.h + padY*2
    ...
    -- Cache
    geo = { x = osc_geo.x + osc_geo.w - padX, y = line1, an = 9, w = 100, h = lineH }
    lo = add_layout("cache")
    lo.geometry = geo
    lo.style = osc_styles.vidtitleBar

    -- Title
    local t_l = osc_geo.x + padX
    local t_r = geo.x - geo.w - padX*2
    geo = { x = t_l, y = geo.y, an = 7, w = t_r - t_l, h = geo.h }
    lo = add_layout("title")
    lo.geometry = geo
    ...
    -- Playlist prev/next
    geo = { x = osc_geo.x + padX + buttonW/2, y = line2 + lineH/2, an = 5, w = buttonW, h = lineH }
    lo = add_layout("pl_prev")
    lo.geometry = geo
    lo.style = osc_styles.smallButtonsRotated

    geo = { x = geo.x + geo.w + padX, y = geo.y, an = geo.an, w = buttonW, h = geo.h }
    lo = add_layout("pl_next")
    lo.geometry = geo
    lo.style = osc_styles.smallButtonsRotated

    -- Playback control buttons
    geo = { x = geo.x + geo.w + padX, y = geo.y, an = 5, w = buttonW, h = geo.h }
    ...
    -- Left timecode
    geo = { x = geo.x + buttonW*1.5 + padX + tcW, y = geo.y, an = 6, w = tcW, h = geo.h }
    ...
    -- Volume
    geo = { x = osc_geo.x + osc_geo.w - padX - buttonW, y = geo.y, an = 4, w = buttonW, h = geo.h }
    ...
    -- Right timecode
    geo = { x = geo.x - buttonW - padX - tcW, y = geo.y, an = 4, w = tcW, h = geo.h }
    ...
    -- Seekbar
    geo = { x = sb_l, y = geo.y, an = 4, w = math.max(0, sb_r - sb_l), h = geo.h }
...
function osc_init()
    ...
    ne.content = function ()
        ...
        if #tracks_osc.audio > 0 then
            local aid = "{\\alpha&FF}0{\\alpha&00}"
            if not (get_track("audio") == 0) then
                aid = get_track("audio")
            end
            nums = " " .. aid .. " ⁄ " .. #tracks_osc.audio
        end
        return ("\238\132\134" .. osc_styles.timecodesBar .. nums)
    end
    ne.eventresponder["mbtn_left_up"] =
    ...
    ne.content = function ()
        ...
        if #tracks_osc.sub > 0 then
            local sid = "{\\alpha&FF}0{\\alpha&00}"
            if not (get_track("sub") == 0) then
                sid = get_track("sub")
            end
            nums = " " .. sid .. " ⁄ " .. #tracks_osc.sub
        end
        return ("\238\132\135" .. osc_styles.timecodesBar .. nums)
    end
    ne.eventresponder["mbtn_left_up"] =
    ...
    ne.content = function ()
        ...
        return (min > 0 and string.format("%sm%02.0fs", min, sec) or string.format("%3.0fs", dmx_cache))
    end
