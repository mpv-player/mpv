...
static void mp_linux_als_destroy(void *p)
{
    struct mp_linux_als *state = p;
...
{
    struct mp_linux_als *state =
        talloc_zero(parent, struct mp_linux_als);

    state->log = mp_log_new(state, global->log, "linux-als");

    talloc_set_destructor(state, mp_linux_als_destroy);

    return state;
}

// Assumes that state->device_dir is NULL
static int acquire_sensor(struct mp_linux_als *state)
{
    ...
    while ((ent = readdir(devices_dir))) {
        ...
        if (!state->device_dir) {
            mp_err(state->log, "Failed to open ambient light sensor %s: %s\n",
                   ent->d_name,
                   mp_strerror(errno));
...
static int scan_from_subfile(struct mp_linux_als *state,
                             const char *subpath,
                             const char *format,
                             void *out)
{
    ...
    readbuf[nread] = 0;
    if (sscanf(readbuf, format, out) != 1)
        return -1;
...
static int read_als_assume_have_sensor(struct mp_linux_als *state,
                                       double *lux)
{
    int illu_raw;
    ...
    *lux = ((double) illu_raw + illu_off) * illu_scl;
    return 0;
}
