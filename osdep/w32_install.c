...
struct w32_install_opts {
    bool install;
    char *rpath;
    bool uninstall;
};

#define OPT_BASE_STRUCT struct w32_install_opts
const struct m_sub_options w32_install_conf = {
    .opts = (const struct m_option[]) {
        {"install", OPT_BOOL(install)},
        {"install-rpath", OPT_STRING(rpath)},
        {"uninstall", OPT_BOOL(uninstall)},
        {0}
    },
    .size = sizeof(struct w32_install_opts)
};
                    ...
                    LPCWSTR value)
{
    ...
    if (status != ERROR_SUCCESS) {
        mp_msg(log, MSGL_ERR,
               "Failed to delete registry %s: %ls\\%ls%ls%ls -> %ld\n", name,
               root, sub_key, value ? L"\\" : L"", value ? value : L"", status);
    }
}
...
static void w32_install(struct MPContext *mpctx)
{
    ...
    mp_msg(log, MSGL_INFO, "Installing mpv for %s...\n",
    ...
    char *rpath = mpctx->opts->w32_install_opts->rpath;
    if (rpath) {
    ...
    // This is the same list as in etc/mpv.desktop, keep them in sync.
    const char *const safe_protocols[] = {
        "ftp",  "http", "https", "mms", "rist",   "rtmp",
        "rtsp", "sftp", "smb",   "srt", "webdav", "webdavs",
    };

    char **protocols = talloc_steal(tmp, stream_get_proto_list());
    char *protocols_str = NULL;
    for (int i = 0; i < MP_ARRAY_SIZE(safe_protocols); ++i) {
        if (str_in_list(bstr0(safe_protocols[i]), protocols)) {
    ...
    reg_add_str(log, root, KEY_MPV_PROG_ID("file"), L"FriendlyTypeName", L"mpv file handler");
    ...
    if (shortcut_path) {
        create_shortcut(log, mpv_path, shortcut_path, NULL, NULL, 0, false);
    ...
    reg_add_str(log, root, KEY_MPV_UNINSTALL, L"UninstallString", L"mpv.exe --no-config --uninstall");
    ...
    mp_msg(log, MSGL_INFO, "mpv installed.\n");

    talloc_free(tmp);
}

static void w32_uninstall(struct MPContext *mpctx)
{
    struct mp_log *log = mp_log_new(NULL, mpctx->log, "win32");

    HKEY root = is_admin() ? HKEY_LOCAL_MACHINE : HKEY_CURRENT_USER;
    mp_msg(log, MSGL_INFO, "Uninstalling mpv for %s...\n",
           root == HKEY_LOCAL_MACHINE ? "all users" : "the current user");
    ...
    if (shortcut_path) {
        ...
        if (!DeleteFileW(shortcut_path))
            mp_msg(log, MSGL_ERR, "Failed to delete shortcut: '%ls'\n",
                   shortcut_path);
        LocalFree(shortcut_path);
    }
    ...
    mp_msg(log, MSGL_INFO, "mpv uninstalled.\n");

    talloc_free(log);
}

bool mp_w32_handle_install(struct MPContext *mpctx)
{
    struct w32_install_opts *opts = mpctx->opts->w32_install_opts;
