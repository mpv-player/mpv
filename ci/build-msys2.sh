...
elif [[ "$SYS" != "clangarm64" ]]; then
    # sanitizers are not supported on stable rust yet so clang64 is excluded
    # clangarm64 is excluded because implib currently seems to produce incorrect
    # implibs for ARM64, see https://github.com/afishhh/subrandr/issues/31
