...
enum option_flags {
    M_OPT_NOCFG     = (1 << 0), // The option is forbidden in config files.
    M_OPT_PRE_PARSE = (1 << 1), // The option should be set during command line pre-parsing
    M_OPT_FILE      = (1 << 2), // The option expects a file name (or a list of file names)
    M_OPT_NOPROP    = (1 << 3), // Do not add as property.
    M_OPT_HAVE_HELP = (1 << 4), // Enable special semantics for some options when parsing the string "help".

    M_OPT_DEFAULT_NAN      = (1 << 5), // type_float/type_double: string "default" is parsed as NaN (and reverse)
    // type time: string "no" maps to MP_NOPTS_VALUE (if unset, NOPTS is
    // rejected) and parsing: "--no-opt" is parsed as "--opt=no"
    M_OPT_ALLOW_NO         = (1 << 6),
    M_OPT_CHANNELS_LIMITED = (1 << 7), // type channels: disallow "auto" (still accept ""), limit list to at most 1 item.
    M_OPT_FIXED_LEN_PRINT  = (1 << 8), // type_float/type_double: controls if pretty print should trim trailing zeros
    M_OPT_OPTIONAL_PARAM   = (1 << 9), // Like M_OPT_TYPE_OPTIONAL_PARAM.

    // The following are also part of the M_OPT_* flags, and are used to update
    // certain groups of options.
    UPDATE_OPT_FIRST       = (1 << 10),
    UPDATE_TERM            = (1 << 10), // terminal options
    UPDATE_SUB_FILT        = (1 << 11), // subtitle filter options
    UPDATE_OSD             = (1 << 12), // related to OSD rendering
    UPDATE_BUILTIN_SCRIPTS = (1 << 13), // osc/ytdl/stats
    ...
    UPDATE_PRIORITY        = (1 << 17), // --priority (Windows-only)
    UPDATE_SCREENSAVER     = (1 << 18), // --stop-screensaver
    UPDATE_VOL             = (1 << 19), // softvol related options
    UPDATE_LAVFI_COMPLEX   = (1 << 20), // --lavfi-complex
    UPDATE_HWDEC           = (1 << 21), // --hwdec
    UPDATE_DVB_PROG        = (1 << 22), // some --dvbin-...
    UPDATE_SUB_HARD        = (1 << 23), // subtitle opts. that need full reinit
    UPDATE_SUB_EXTS        = (1 << 24), // update internal list of sub exts
    UPDATE_VIDEO           = (1 << 25), // force redraw if needed
    UPDATE_VO              = (1 << 26), // reinit the VO
    UPDATE_CLIPBOARD       = (1 << 27), // reinit the clipboard
    UPDATE_OPT_LAST        = (1 << 27),
};
