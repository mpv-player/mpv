...
if(has_audio==4){
  ...
  if(init_audio_codec()){
    ...
    if(a_buffer_size<avi_header.audio_out_minsize+AUDIO_BUFF)
        a_buffer_size=avi_header.audio_out_minsize+AUDIO_BUFF;
  } else {
...
while(!eof){
float old_vframe=v_frame;
/*========================== DECODE AUDIO ============================*/

if(has_audio){
  unsigned int t=GetTimer();
  int a_play_bytes=(v_frame-a_frame)*(float)(MP3_samplerate*MP3_channels*MP3_bps);

    a_play_bytes&=~15;
//    a_play_bytes=((a_play_bytes/OUTBURST)+2)*OUTBURST;
//    printf("a_play_bytes=%d\n",a_play_bytes);
    if(a_play_bytes>AUDIO_BUFF) a_play_bytes=AUDIO_BUFF;

  current_module="decode_audio";   // Enter AUDIO decoder module

  // Update buffer if needed
  while(a_buffer_len<a_play_bytes && !d_audio->eof){
    switch(has_audio){
...
if(has_audio){
  int len=(old_vframe-a_frame)*(float)(MP3_samplerate*MP3_channels*MP3_bps);
  len&=~15;
  if(len>a_buffer_len) len=a_buffer_len;
...
#ifdef USE_XMMP_AUDIO
    pSound->Write( pSound, a_buffer, len );
#else
#ifdef SIMULATE_ALSA
    fake_ALSA_write(audio_fd,a_buffer,len); // for testing purposes
#else
    write(audio_fd,a_buffer,len);
#endif
#endif

    a_buffer_len-=len;
    memcpy(a_buffer,&a_buffer[len],a_buffer_len);

  a_frame+=len/(float)(MP3_samplerate*MP3_channels*MP3_bps);
  a_pts+=len/(float)(MP3_samplerate*MP3_channels*MP3_bps);
  time_frame+=len/(float)(MP3_samplerate*MP3_channels*MP3_bps);

}

/*========================== UPDATE TIMERS ============================*/

    current_module="alsa_timer";

  if(alsa){
    // Use system timer for sync, not audio card/driver
    time_frame-=GetRelativeTime();
//    printf("time_frame=%5.3f\n",time_frame);
    if(time_frame<0 || time_frame>0.1){
      time_frame=0;
    } else {
...
//      if(time_frame>0.02) usleep(1000000*(time_frame)); // sleeping if >20ms
        while(time_frame>0.0001){
            if(time_frame>0.021) usleep(1000000*(time_frame-0.020)); else
            if(time_frame>0.011) usleep(0);
//          usleep(1000);
            time_frame-=GetRelativeTime();
        }
//        printf("%8.3f    \n",time_frame*1000);
