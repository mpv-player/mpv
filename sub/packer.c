...
#if HAVE_SUBRANDR
static inline bool clip_instance(struct sbr_output_instance *instance) {
    return instance->width == 0 || instance->height == 0 ||
           instance->x + (int)instance->width <= 0 ||
           instance->y + (int)instance->height <= 0;
}
                          ...
                          struct sbr_instanced_raster_pass *pass)
{
    ...
    for (int n = 0; n < res->num_parts; n++) {
        struct sub_bitmap *b = &res->parts[n];
        sbr_output_image_draw_to(b->subrandr.image, pass, b->src_x, b->src_y, base,
                                res->packed_w, res->packed_h, pixel_stride);
    ...
    for (struct sbr_output_instance *instance = instances; instance; instance = instance->next) {
        if (clip_instance(instance) || !instance->base->user_data)
            continue;
        ...
        // Take destination coordinates from the instance
        inst_b->x = instance->x, inst_b->y = instance->y;
        inst_b->dw = instance->width, inst_b->dh = instance->height;
        // and source parameters from the base image and bitmap
        inst_b->w = image_b->w, inst_b->h = image_b->h;
        inst_b->src_x = image_b->src_x, inst_b->src_y = image_b->src_y;
        inst_b->bitmap = image_b->bitmap, inst_b->stride = byte_stride;
        res->num_parts++;
    }
