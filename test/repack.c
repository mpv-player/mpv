...
static int try_repack(FILE *f, int imgfmt, int flags, int not_if_fmt)
{
    ...
    if (un && pa) {
        mp_assert(mp_repack_get_format_src(pa) == mp_repack_get_format_dst(un));
        mp_assert(mp_repack_get_format_src(un) == mp_repack_get_format_dst(pa));
        mp_assert(mp_repack_get_align_x(pa) == mp_repack_get_align_x(un));
        mp_assert(mp_repack_get_align_y(pa) == mp_repack_get_align_y(un));
    }
    ...
    if (a == imgfmt && b == imgfmt) {
        mp_assert(is_true_planar(imgfmt));
        // (note that we require alpha-enabled zimg)
        mp_assert(mp_zimg_supports_in_format(imgfmt));
        mp_assert(un && pa);
        talloc_free(pa);
        talloc_free(un);
        return b;
                               ...
                               enum pl_color_levels levels)
{
    ...
    mp_assert(src->params.repr.sys == csp);
    mp_assert(src->params.repr.levels == levels);
