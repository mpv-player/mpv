...
{
    mp_assert(ref->imgfmt == new->imgfmt);
    mp_assert(ref->w == new->w);
    mp_assert(ref->h == new->h);

    mp_assert(ref->fmt.flags & MP_IMGFLAG_BYTE_ALIGNED);
    mp_assert(ref->fmt.bpp[0]);
...
void repack_test_run(struct scale_test *stest)
{
    ...
    for (int a = 0; a < num_imgfmts; a++) {
        ...
        mp_assert(stest->fns->supports_fmts(stest->fns_priv, test_img->imgfmt, mpfmt));
