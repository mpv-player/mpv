...
``opengl``
    ...
    ``pre-shader=<file>``, ``post-shader=<file>``, ``scale-shader=<file>``
        ...
        These files should contain raw GLSL (so no function definitions), and
        they operate on linear RGB (with the exception of ``scale-shader``,
        which can operate on sigmoidized RGB if ``sigmoid-upscaling`` is
        enabled, and chroma if ``cscale=custom`` is used).
