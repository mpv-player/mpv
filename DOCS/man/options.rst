...
``--hwdec-threads=<N>``
    Number of threads used for hardware decoding (default: 2). This as opposed
    to vd-queue enables frame and slice threading in libavcodec,. This can help
    with pipelining the decoding process, and can improve performance. Exact
