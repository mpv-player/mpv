...
``current-window-scale`` (RW)
    ...
    It is possible to write to this property, but it is deprecated. When doing
    so, it has the same behavior as writing ``window-scale``. Note that writing
    to ``current-window-scale`` will not affect the value of ``window-scale``.
