...
function print_state(params)
	if params then
		mp.osd_message(script_name..":\n"
						...
						.."target = "..tostring(params["target"]*100).."%")
	else
		mp.osd_message(script_name..":\noff")
	end
end
...
function drc_scale_target(factor)
	modify_or_create_af(
		function (afs)
			oldVal = afs[i]["params"]["target"]
			newVal = math.min(math.max(oldVal+factor, 0), 1)
			
			afs[i]["params"]["target"] = tostring(newVal)
			print_state(afs[i]["params"])
...
mp.add_key_binding("ctrl+9", "drc_quieter", drc_quieter_handler, {repeatable = true})
mp.add_key_binding("ctrl+0", "drc_louder",  drc_louder_handler,  {repeatable = true})
