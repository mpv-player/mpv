...
impl Bstr {
    ...
    const fn empty() -> Self {
        Bstr { ptr: std::ptr::null_mut(), len: 0 }
    }

    fn splice(&self, start: c_int, end: c_int) -> Bstr {
...
fn guess_lang(name: &Bstr) -> Option<(usize, &str, &str, bool)>
{
    if name.len == 0 {
        return None;
    }
    ...
    let (pos, _, delimiter) = extract_tag(&mut iter, pos, s)?;
    if delimiter != "." {
        return None; // No extension found
    }

    let (pos, tag, delimiter, hi) = extract_tag(&mut iter, pos, s)
        .and_then(|(pos, tag, delimiter)| match tag {
            "hi" | "sdh" | "cc" => extract_tag(&mut iter, pos, s)
                .map(|(pos, tag, delimiter)| (pos, tag, delimiter, true)),
            _ => Some((pos, tag, delimiter, false)),
        })?;

    tag.parse::<LanguageTag>().ok()
        .and_then(|tag| tag.canonicalize().unwrap_or(tag).primary_language().parse::<Language>().ok())
        .is_some()
        .then_some((pos, tag, delimiter, hi))
}
...
/// * `hearing_impaired` - An optional pointer to store whether the filename
///   indicates hearing-impaired content.
///   Set to true if tags like "hi", "sdh", or "cc" are found, false otherwise.
///
/// # Returns
                                              ...
                                              hearing_impaired: *mut bool) -> Bstr
{
    if let Some((pos, tag, delimiter, hi)) = guess_lang(&name) {
        ...
        assign_ptr(hearing_impaired, false);
        Bstr::empty()
    }
}
